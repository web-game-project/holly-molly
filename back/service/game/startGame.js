...
module.exports.startGame = async (req, res, next) => {
    ...
    try {
        ...
        let user_list = await assignRoleAndOrder(game_idx, roleList, orderList, memberIdxList, memberCount);
...
const getRoleAndOrder = (memberCount) => {
    ...
    for(let i=1; i<=memberCount; i++){
        if(i == random_idx)
            roleList.push(1); //"citizen"
        else
            roleList.push(0); //"ghost"
        orderList.push(i);
    } 
...
const assignRoleAndOrder = async (gameIdx, roleList, orderList, memberIdxList, memberCount) => {
    let memberInfoList = [];
    ...
    try {
        for(let i=0; i<memberCount; i++){
            ...
            memberInfoList.push({user_idx: wrm_user_idx, game_member_order: orderList[i], user_color});
